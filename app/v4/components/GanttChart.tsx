...
function getBarColor(project: Project): string {
              ...
              <div key={project.id}>
                ...
                >
                  ...
                  <div className="relative flex-shrink-0 h-full" style={{ width: tw }}>
                    ...
                    <div
                      className={`absolute rounded-md overflow-hidden opacity-85 ${getBarColor(project)}`}
                      style={{ left: pLeft, width: pWidth, top: "50%", transform: "translateY(-50%)", height: 20 }}
                    >
                      {pWidth >= 30 && (
                        <span className="absolute left-1.5 top-1/2 -translate-y-1/2 text-[10px] font-semibold text-white/90 pointer-events-none whitespace-nowrap">
                          {project.progress}%
                        </span>
                      )}
                    </div>
